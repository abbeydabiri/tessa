...
func apiSecure(httpRes http.ResponseWriter, httpReq *http.Request) (message *Message) {
	...
	if apiBlock("admin", claims) && apiBlock("support", claims) && apiBlock("staff", claims) &&
		apiBlock("supplier", claims) && apiBlock("merchant", claims) && apiBlock("customer", claims) {
...
func apiSecurePost(httpRes http.ResponseWriter, httpReq *http.Request) (tableMap map[string]interface{}, message *Message) {
	...
	if claims["ID"] != nil {
		if tableMap["ID"] == nil {
			tableMap["Createdby"] = uint64(claims["ID"].(float64))
		} else {

			switch tableMap["ID"].(type) {
			case uint64:
				if tableMap["ID"].(uint64) == uint64(0) {
					tableMap["Createdby"] = uint64(claims["ID"].(float64))
				}
				tableMap["Updatedby"] = uint64(claims["ID"].(float64))
			case float64:
				if uint64(tableMap["ID"].(float64)) == uint64(0) {
					tableMap["Createdby"] = uint64(claims["ID"].(float64))
				}
				tableMap["Updatedby"] = uint64(claims["ID"].(float64))
			}
...
func apiSecureGet(httpRes http.ResponseWriter, httpReq *http.Request) (formSearch *database.SearchParams, message *Message) {
	...
	if apiBlock("admin", claims) && apiBlock("support", claims) && apiBlock("customer", claims) {
		apiBlockResponse(httpRes)
		return
	}
